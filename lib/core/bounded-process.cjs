...
function runBoundedProcess(options) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      // On Windows, Node (post CVE-2024-27980) refuses to spawn .cmd/.bat
      // wrappers and extensionless shell shims with shell:false. We only opt
      // into shell mode for those known wrapper extensions, and spawn still
      // individually quotes each argv element — so the prompt (delivered on
      // stdin, never argv) cannot be reinterpreted as a shell command. The
      // adapter-supplied argv contains only fixed flags; no user-controlled
      // token reaches the shell.
      const isWindowsWrapper =
        process.platform === 'win32' && /\.(cmd|bat|ps1)$/i.test(options.program);
