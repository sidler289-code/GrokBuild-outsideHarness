...
function which(name, searchDirs) {
  ...
  for (const dir of searchDirs) {
    ...
    if (process.platform === 'win32') {
      // Do not treat extensionless POSIX/npm shims as native Windows
      // executables. Prefer a native .exe and leave shell wrappers to the
      // fail-closed probe below.
    ...
    } else if (isExecutableSync(direct)) {
      hits.push(direct);
    }
  }
...
function defaultRun(candidate, args, { timeoutMs }) {
  return new Promise((resolve) => {
    ...
    try {
      if (process.platform === 'win32' && /\.(cmd|bat|ps1)$/i.test(candidate.path)) {
        settle({
          exitCode: null,
          startError: 'unsafe_windows_wrapper',
          stdout: '',
          stderr: '',
          timedOut: false,
        });
...
function candidateIdentityMatches(harnessId, candidate, probe) {
  const text = `${probe.stdout || ''}\n${probe.stderr || ''}`;
  // `agent` is an extremely generic compatibility alias. Require an
  // identity marker so unrelated products such as Grok agent.exe cannot be
  // invoked as Cursor.
  if (harnessId === 'cursor' && candidate.binaryName === 'agent') {
    return /\bcursor(?:-agent)?\b/i.test(text);
  }
  return true;
}
...
async function detectHarness(harnessId, options = {}) {
  ...
  if (!probe.startError && !probe.timedOut && !candidateIdentityMatches(harnessId, candidate, probe)) {
    return {
      harnessId,
      available: false,
