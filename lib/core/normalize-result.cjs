...
const REVIEWERS = new Set(['claude', 'codex', 'opencode', 'cursor']);
...
function collectJsonCandidates(output) {
  ...
  // Preserve the strict, documented path first.
  add(output);

  // Real reviewers sometimes add prose before a JSON/untagged Markdown fence.
  const fencePattern = /```(?:json)?[ \t]*\r?\n([\s\S]*?)\r?\n[ \t]*```/gi;
  for (const match of output.matchAll(fencePattern)) {
    add(match[1]);
  }

  // Final fallback: inspect balanced JSON-looking objects embedded in prose.
  // Braces inside quoted JSON strings do not affect the balance counter.
  let objectStart = -1;
  let depth = 0;
  let inString = false;
  let escaped = false;
  for (let index = 0; index < output.length; index += 1) {
    ...
    if (character === '"' && depth > 0) {
      ...
      depth -= 1;
      if (depth === 0) {
        add(output.slice(objectStart, index + 1));
        objectStart = -1;
      }
    }
  }
...
function normalizeReviewResult({ task, role, reviewer, processResult }) {
  ...
  for (const candidate of collectJsonCandidates(processResult.stdout)) {
    try {
      const value = JSON.parse(candidate);
      parsedAnyJson = true;
      if (validateReviewResult(value, { task, role, reviewer })) {
        parsed = value;
        break;
      }
    } catch {
      // Try the next bounded candidate before classifying the output.
    }
  }

  if (parsed === undefined && !parsedAnyJson) {
    return baseEnvelope({
      task,
      role,
      reviewer,
