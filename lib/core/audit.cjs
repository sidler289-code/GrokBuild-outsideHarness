...
const { loadUserConfig, loadProjectConfig } = require('./config.cjs');
const {
  TestExecutionError,
...
} = require('./test-execution.cjs');
const { detectHarness, detectLegacyReviewers } = require('./discovery.cjs');
...
const { TESTS_POLICY, LEGACY_TESTS_POLICY } = require('../policies/tests.cjs');

const POLICIES = { plan: PLAN_POLICY, code: CODE_POLICY, tests: TESTS_POLICY, security: SECURITY_POLICY };
const BUNDLED_EMPTY_MCP_CONFIG = path.join(__dirname, '..', '..', 'config', 'empty-mcp.json');
const BUNDLED_RESULT_SCHEMA = path.join(__dirname, '..', '..', 'skills', 'cross-harness-review', 'schemas', 'review-result-v2-codex.schema.json');
...
function testEnvelope({ reviewer, status, summary, testExecution }) {
...
function resolveTestExecutionContext({ repoRoot, userConfig, projectConfig }) {
  const policyCheck = requireEnabledUserPolicy(userConfig);
  if (!policyCheck.allowed) {
    return {
      error: policyCheck.reason,
      summary: policyCheck.reason === 'policy_blocked'
        ? 'Direct test execution is disabled by testsExecution.enabled.'
        : 'Configured user testsExecution policy is required.',
      testExecution: {
        attempted: false,
  ...
  try {
    resolvedProjectConfig = projectConfig ?? loadProjectConfig(repoRoot);
    if (!resolvedProjectConfig) {
      return {
        error: 'configuration_required',
        summary: 'Project .cross-harness-review.json test commands are required.',
        testExecution: {
          attempted: false,
          verifiedByEvents: false,
          outcome: 'not_run',
    ...
    return {
      ...
      projectConfig: resolvedProjectConfig,
      userPolicy: policyCheck.policy,
    };
  } catch (error) {
    const code = error instanceof TestExecutionError ? error.code : 'configuration_required';
    return {
      error: code === 'policy_blocked' ? 'policy_denied' : 'configuration_required',
      summary: `Invalid project test configuration: ${error.message}`,
      testExecution: {
        attempted: false,
        verifiedByEvents: false,
        outcome: 'not_run',
...
function attachTestEvidence(envelope, evidence, before, after) {
  const warnings = [
    ...(Array.isArray(envelope.diagnostics.warnings) ? envelope.diagnostics.warnings : []),
  ...
  return {
    ...
    testExecution: {
      ...
      verifiedByEvents: evidence.verifiedByEvents,
      outcome: evidence.outcome,
      commands: evidence.commands,
      workspaceChanged: before.join('\0') !== after.join('\0'),
      workspaceChanges: after,
    },
    diagnostics: warnings.length > 0 ? { ...envelope.diagnostics, warnings } : envelope.diagnostics,
  };
}
...
async function runSingleReview({
  ...
  scopeSelector,
  program,
  emptyMcpConfigPath = BUNDLED_EMPTY_MCP_CONFIG,
  resultSchemaPath = BUNDLED_RESULT_SCHEMA,
  ...
  userConfig,
  projectConfig,
  legacyStaticTests = false,
}) {
  const policy = task === 'tests' && legacyStaticTests ? LEGACY_TESTS_POLICY : POLICIES[task];
  if (!policy) {
  ...
  if (task === 'tests' && !legacyStaticTests) {
    const resolvedUserConfig = userConfig || loadUserConfig();
    testContext = resolveTestExecutionContext({ repoRoot, userConfig: resolvedUserConfig, projectConfig });
    if (testContext.error) {
      return testEnvelope({
        reviewer,
        status: testContext.error === 'policy_blocked' ? 'policy_denied' : testContext.error,
        summary: testContext.summary,
        testExecution: testContext.testExecution,
      });
    }
  }
  ...
  if (task === 'tests' && !legacyStaticTests) {
    try {
      workspaceBefore = await snapshotGitStatus({ repoRoot, runImpl });
    } catch (error) {
      return testEnvelope({
        reviewer,
        status: 'process_failed',
        summary: `Unable to observe the workspace before tests: ${error.message}`,
        testExecution: {
          attempted: false,
          verifiedByEvents: false,
  ...
  const prompt = buildPrompt({
    ...
    planDigest,
    snapshot,
    approvedCommands: testContext?.approvedCommands,
    extraInstructions,
    legacyStaticTests,
  });
  ...
  const invocation = adapter.buildInvocation({
    program,
    taskContext: { prompt, repoRoot, emptyMcpConfigPath, resultSchemaPath, approvedCommands: testContext?.approvedCommands },
  });
  ...
  if (task === 'tests' && !legacyStaticTests) {
    try {
      workspaceAfter = await snapshotGitStatus({ repoRoot, runImpl });
    } catch (error) {
      return testEnvelope({
        reviewer,
        status: 'process_failed',
        summary: `Unable to observe the workspace after tests: ${error.message}`,
        testExecution: {
          attempted: false,
          verifiedByEvents: false,
          outcome: 'inconclusive',
  ...
  if (task === 'tests' && !legacyStaticTests) {
    let events = [];
    try {
      const parsedEvents = adapter.parseEvents({ processResult, recorder: eventRecorder.snapshot() });
      events = Array.isArray(parsedEvents) ? parsedEvents : [];
    } catch {
      events = [];
    }
    const evidence = verifyTestExecutionEvents({
      events,
      approvedCommands: testContext.approvedCommands,
      repoRoot,
      maxOutputBytes: testContext.userPolicy.maxOutputBytes,
    });
    envelope = attachTestEvidence(envelope, evidence, workspaceBefore, workspaceAfter);
  }
  ...
  return envelope;
    ...
    // Cleanup also runs when the process launcher, event parser, normalizer,
    // or post-test workspace observation fails.
    try {
      await adapter.cleanup({ cleanupPaths: invocation.cleanupPaths });
    } catch {
      // Best-effort; never mask the original result or error.
    }
  }
}
 ...
 */
async function resolveReviewersForTask({ task, explicitReviewer, userConfig, detectImpl = null }) {
  if (explicitReviewer) {
    const detection = detectImpl ? await detectImpl(explicitReviewer) : await detectHarness(explicitReviewer);
    const detected = Array.isArray(detection) ? detection : [detection];
  ...
  if (!userConfig.configured) {
    const detected = detectImpl ? await detectImpl() : await detectLegacyReviewers();
  ...
  const detection = detectImpl ? await detectImpl(harnessId) : await detectHarness(harnessId);
  const detected = Array.isArray(detection) ? detection : [detection];
...
async function runAudit(options) {
  ...
  if (options.task === 'tests') {
    const userConfig = options.userConfig || loadUserConfig();
    if (!userConfig.configured) {
      // 0.1 compatibility: tests is a read-only strategy audit until setup
      // persists explicit host-test authorization.
      resolvedOptions = { ...options, userConfig, legacyStaticTests: true };
    } else {
      const testContext = resolveTestExecutionContext({
        repoRoot: options.repoRoot,
        userConfig,
        projectConfig: options.projectConfig,
      });
      if (testContext.error) {
        const reviewer = options.explicitReviewer || userConfig.config?.roles?.tests || 'claude';
        return {
          aggregateStatus: 'mixed',
          reason: testContext.error,
          unavailable: [],
      ...
      resolvedOptions = { ...options, userConfig, projectConfig: testContext.projectConfig };
    }
  }
  const { reviewers, reason, unavailable } = await resolveReviewersForTask(resolvedOptions);
  if (reviewers.length === 0) {
  ...
  for (const { reviewer, program } of reviewers) {
    const envelope = await runSingleReview({ ...resolvedOptions, reviewer, program });
    envelopes.push(envelope);
  }

  return {
    aggregateStatus: envelopes.every((envelope) => {
      if (envelope.status !== 'success') {
        return false;
      }
      return options.task !== 'tests' ||
        resolvedOptions.legacyStaticTests === true ||
        (envelope.testExecution.verifiedByEvents && envelope.testExecution.outcome === 'passed');
    }) ? 'all_success' : 'mixed',
    reason,
    unavailable,
    envelopes,
