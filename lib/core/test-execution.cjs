...
const SAFE_OPTIONAL_ENV_NAMES = new Set([
  'CI',
  'NODE_ENV',
  'TZ',
  'LANG',
...
function safeTestEnvironment({ environmentAllowlist = [], env = process.env } = {}) {
  const unsafeNames = environmentAllowlist.filter((name) => !SAFE_OPTIONAL_ENV_NAMES.has(name));
  if (unsafeNames.length > 0) {
    throw new TestExecutionError(
      'unsafe_environment',
      `environmentAllowlist contains unsupported variables: ${unsafeNames.join(', ')}.`
    );
  }
