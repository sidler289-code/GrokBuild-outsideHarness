 ...
 * mismatch.
 */
function assertRoleMappingPassesGates(
  roles,
  capabilities,
  { testsExecutionEnabled = true } = {}
) {
  for (const role of ROLES) {
    if (role === 'tests' && !testsExecutionEnabled) {
      continue;
    }
 ...
 */
function routeConfigured(roles, capabilities, options = {}) {
  assertRoleMappingPassesGates(roles, capabilities, options);
...
async function computeRolesReport({ userConfig, capabilities = null, detectImpl = detectLegacyReviewers } = {}) {
  ...
  assertRoleMappingPassesGates(userConfig.config.roles, caps, {
    testsExecutionEnabled: userConfig.config.testsExecution?.enabled !== false,
  });
