...
function buildInvocation({ program, taskContext }) {
  ...
  // Inherit only a minimal environment. We redirect Claude's config discovery
  // at a fresh empty temp directory so user hooks, agents, CLAUDE.md and
  // stored auth cannot load for this audit turn. An empty string env var is
  // treated as "unset" by Claude Code, so we MUST point at a real (empty) dir.
  const isolatedConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'chr-claude-cfg-'));
  const env = {
    ...
    DISABLE_TELEMETRY: '1',
    CLAUDE_CONFIG_DIR: process.env.CROSS_HARNESS_CLAUDE_CONFIG_DIR || isolatedConfigDir,
  };
...
function cleanup(runContext = {}) {
  ...
  for (const p of paths) {
    try {
      ...
      // Best-effort cleanup; bounded-process already terminated the child.
    }
  }
