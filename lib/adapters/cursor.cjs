...
// UNREGISTERED PREPARATION: retained for future hardening work only. It is not
// exported by lib/adapters/index.cjs and cannot be discovered or assigned.
...
function writePermissionConfigFile(configDir) {
  const cursorDir = path.join(configDir, '.cursor');
  fs.mkdirSync(cursorDir, { recursive: true });
  const target = path.join(cursorDir, 'cli-config.json');
...
function buildInvocation({ program, taskContext }) {
  ...
  writePermissionConfigFile(isolatedConfigDir);

  const env = {
    PATH: process.env.PATH || '',
    HOME: isolatedConfigDir,
    USERPROFILE: isolatedConfigDir,
...
function parseJsonLines(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    try {
      ...
      // Ignore non-event lines; final result extraction still fails closed.
    }
  }
  return events;
}
...
function normalizeFinalResult({ task, role, reviewer, processResult }, normalizer) {
  ...
  const terminal = [...events].reverse().find((event) =>
    event && event.type === 'result' && event.subtype === 'success' && typeof event.result === 'string'
  );
  const normalizedProcess = terminal ? { ...processResult, stdout: terminal.result } : processResult;
